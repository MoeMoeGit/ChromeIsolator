...
新增“采集模式”。启用后只开放 `41000 + 环境编号` 的本机 CDP 端口，并显式绑定 `127.0.0.1`。不勾选差异模式时，不启动 `FingerprintInjector`，不覆盖 navigator 硬件参数。

设置入口命名为“采集与差异模式”，但 UI 层级以采集模式为主路径：摘要优先显示采集启用数量，二级窗口提供“启用采集 / 关闭采集”批量操作；差异模式只保留短标签和说明文案，避免用户误以为日常采集需要开启差异模式。
...
继续使用 WiX / MSI，不迁移安装器工具。安装目录固定为 `%ProgramFiles%\ChromeIsolator`，不向普通用户暴露目录选择。安装流程使用自定义最小 UI：欢迎页、进度页、完成页。维护 / 卸载流程使用清晰的维护说明、卸载说明、进度页和完成页。程序运行中时通过 `util:CloseApplication` 提示用户从系统托盘图标右键退出，再点击重试继续安装、升级或卸载。安装完成页默认勾选“启动浏览器多开”，安装器继续同步创建开始菜单和桌面快捷方式。卸载默认保留 `%LOCALAPPDATA%\ChromeIsolator` 下的环境数据。

WiX 是第三方开源安装器工具链，但对用户可见的品牌内容必须替换为项目自有内容：安装器产品名使用“浏览器多开”，欢迎页 / 维护页 / 卸载页文案使用中文自定义资源，横幅和背景图使用 `installer/assets/` 下的项目自有位图，不保留 WiX 默认图片作为最终发布界面。
...
5. 完成页默认提供“启动浏览器多开”。
6. 保留桌面快捷方式和开始菜单快捷方式。
7. 安装器可使用 WiX 技术底座，但默认图片和面向用户的主要文案必须替换为项目自有内容。
...
| 日期 | 变更内容 |
|------|----------|
| 2026-07-08 | 补充决策 7：安装器完成页默认启动应用，保留桌面和开始菜单快捷方式，替换 WiX 默认图片和主要文案 |
| 2026-07-08 | 更新决策 9：采集与差异模式 UI 改为采集优先，差异模式降为低频并列开关，并增加批量采集操作 |
