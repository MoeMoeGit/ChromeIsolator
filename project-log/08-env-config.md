...
- 基础模式：不开放调试端口。
- 采集模式：开放 `41000 + 环境编号`，例如 `p3` 为 `41003`，只绑定 `127.0.0.1`，供本机采集工具连接。
- 差异模式：未启用采集模式时开放 `40000 + 环境编号`，用于 ChromeIsolator 自身注入轻量 navigator 参数。
- 采集模式和差异模式同时启用：使用采集模式端口 `41000 + 环境编号`，并继续执行差异注入。
...
| 日期 | 变更内容 | 原因 |
|------|----------|------|
| 2026-07-07 | 补充采集模式端口约定：`41000 + 环境编号`，只用于本机 CDP 连接；差异模式端口仍为 `40000 + 环境编号` | 支撑 cscout 多账号采集，不混用差异模式 |
