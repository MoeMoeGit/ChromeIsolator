...
## 2026-08-24（cscout 后台标签页集成验证）

**触发原因**：实际采集不应要求用户提前打开罗盘页面，也应尽量避免切走用户正在使用的标签页。

**验证结果**：ChromeIsolator 无需新增控制 API；环境只负责开放 `41000 + 环境编号` 的本机 CDP 通道，cscout 通过 browser WebSocket 调用 `Target.createTarget(background=true)`。环境3（41003）真实条件测试成功创建、导航并保持 hidden；随后后台采集抖音上个完整自然周 166 条/9 页，接口总数一致且 `complete=true`，最后关闭测试 target。全程未读取、领取或上传 CCO 生产任务。

**文档修正**：补充 cscout 自动后台建页职责，并修正早期仍把调试端口描述为“仅差异模式开放”的两处旧说明。
...
**触发原因**：用户要求将版本统一推进到 1.7.1，提交 GitHub，打新 tag，并触发 1.7.1 版本构建。

**修改内容**：

1. `Directory.Build.props` — 版本号从 `1.7.0` 推进到 `1.7.1`，同步 Assembly / File / Informational 版本。
2. `README.md` — 当前版本、发布产物文件名和版本 tag 示例统一改为 `1.7.1`。
3. `project-log/05-current-status.md` — 当前版本更新为 `V1.7.1 待发布`，同步当前阶段说明。

**遇到的问题**：

- 该轮本身没有新的实现风险，主要是把当前已完成的功能和文档一起收口到新版本号。

**解决方式**：

- 仅更新版本元数据和当前状态说明，保持历史日志和既有设计记录不变。
...
**触发原因**：用户希望主窗口尺寸、位置和左右栏拖动位置能在重启后自动恢复，避免每次重新拖拽。

**修改内容**：

1. `AppConfig.cs` — 新增主窗口左上角、宽高、最大化状态和左栏宽度配置项。
2. `MainWindow.xaml` — 左右栏列和 GridSplitter 增加命名，便于恢复和保存布局。
3. `MainWindow.xaml.cs` — 增加窗口位置 / 尺寸 / 左栏宽度的恢复与节流保存，关闭时再写回一次，且对最小化状态做保护。
4. `MainViewModel.cs` — 增加主窗口布局持久化读取 / 写入入口，并让托盘唤醒只在最小化时恢复正常态。
5. `App.xaml.cs` — 启动时调用窗口布局恢复。
6. `project-log/05-current-status.md` — 补充本次功能状态。

**遇到的问题**：

- 左栏宽度如果在大屏上拖得很宽，直接恢复到小屏可能挤压右侧详情区。

**解决方式**：

- 根据当前窗口宽度、边距、分隔条和右栏最小宽度对左栏做上限约束，避免恢复后 UI 失衡。
