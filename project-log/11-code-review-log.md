...
- **严重程度**：中
- **状态**：待修复
- **位置**：`src/ChromeIsolator.App/Services/ChromeManager.cs`
- **描述**：在执行关闭操作时，代码使用了 `TryCloseBrowserViaCdpAsync(port.Value).Wait(...)` 和 `injector.DisposeAsync().AsTask().Wait(...)`。强制使用 `.Wait()` 同步阻塞当前线程。在 `StopAll` 时，若同时停止大量环境，会占用大量线程池资源，导致应用响应变慢。
- **修复计划**：将 `ChromeManager.Stop` 彻底重构为 `StopAsync`，并用 `Task.WhenAll` 优化批量关闭。
...
- **状态**：待修复
- **位置**：`src/ChromeIsolator.App/Services/ChromeManager.cs`
- **描述**：如果 Chrome 拒绝响应，最后会执行 `process.Kill(entireProcessTree: true)`。由于未在该处使用 `try-catch` 捕获，异常会直接向外抛出。
- **修复计划**：补充 `Process.Kill` 的安全异常捕获。

#### 问题 3：官方 Chrome 下载缺少网络容错

- **类型**：可用性风险
- **严重程度**：高
- **状态**：待修复
- **位置**：`src/ChromeIsolator.App/Services/ChromeManager.cs`
- **描述**：使用 `HttpClient` 下载 100MB+ 的 MSI 安装包时没有任何自动重试或断点机制，若遇到瞬时的网络波动，下载会直接抛出异常中断。
- **修复计划**：增加 Chrome 下载的重试逻辑。

#### 问题 4：单实例外部链接转发的超时阈值偏紧

- **类型**：可用性风险
- **严重程度**：低
- **状态**：待修复
- **位置**：`src/ChromeIsolator.App/App.xaml.cs`
- **描述**：传递外部链接时，最大重试逻辑是 `5 次 x 150ms` 的休眠，加管道连接本身的短超时。高负载时容易超时。
- **修复计划**：延长重试次数和超时时间。
---
...
| 日期 | 变更内容 | 原因 |
|------|----------|------|
| 2026-06-13 | 新增第十二轮复核，提出停止异步化、进程 Kill 容错、MSI 下载重试以及单实例超时调整 | 提升稳定性和用户体验 |
