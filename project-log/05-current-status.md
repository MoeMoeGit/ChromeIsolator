...
> **最后更新**：2026-05-25
> **最后更新人**：Codex
> **最近开发日志**：`06-dev-log.md` 中的 2026-05-25（V1.7.0 发布触发）
> **当前可信度**：Windows 本地 `dotnet build` 通过，待默认浏览器注册和真实外部链接行为验证

## 当前版本

**V1.7.0 待发布** — 在 V1.6.9 基础上新增环境备注和外部链接接收能力，并继续保留默认基础模式 / 可选差异模式设计；当前版本继续完善详情面板和差异模式管理交互。

## 当前阶段

当前重点是将 1.7.0 版本提交、打 tag 并触发 GitHub Actions 构建，然后等待 Windows 设备执行默认浏览器注册和真实外部链接行为验证。
