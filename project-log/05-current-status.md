...
> **最后更新**：2026-07-08
> **最后更新人**：Codex
> **最近开发日志**：`06-dev-log.md` 中的 2026-07-08（采集与差异模式入口文案和批量操作优化）
> **当前可信度**：采集模式、采集与差异模式窗口布局、安装器构建和版本号已完成本机构建验证；真实抖音后台采集需由 cscout + Playwright 实机验证。

## 当前版本

**V1.7.8 待发布** — 在 V1.7.7 基础上优化采集与差异模式入口，并继续包含安装器产品体验优化：采集模式作为主路径、差异模式降为低频开关，新增批量启用 / 关闭采集操作；完成页默认启动应用，品牌名统一为“浏览器多开”，替换 WiX 默认图片，强化卸载保留数据和运行中退出提示，补充 README / Release 安装指引。

## 当前阶段

V1.7.8 安装器体验优化和采集与差异模式入口优化已完成本地构建验证。下一步是推送 `main` 和 `v1.7.8` tag，触发 GitHub Actions 构建；随后由 cscout 连接采集模式环境验证抖音后台真实采集。
...
- 已由 cscout 在环境3（41003）实测 browser CDP 后台 target：可自动新建、导航、保持 hidden 并关闭，不要求用户预开页面；进一步完成抖音上个完整自然周 166 条/9 页采集且覆盖完整，测试未触发 CCO 任务领取或上传。
- 已保持差异模式 `EnableEnvironmentVariation` 独立：不勾选差异模式时不会注入 `navigator.hardwareConcurrency` / `navigator.deviceMemory`。
- 已更新采集与差异模式二级窗口：采集模式作为主路径短标签展示，差异模式保留为并列低频开关；运行中的环境仍需关闭后切换。
- 已优化采集与差异模式窗口布局：环境名和运行提示单独一行，模式开关垂直排列，并新增可切换环境的批量启用 / 关闭采集操作，避免多语言长文案挤压环境名。
...
- 已完成 `1.7.7` 安装器体验版本号推进记录，本轮继续以 `1.7.8` 发布采集与差异模式入口优化。
- 已将版本号推进到 `1.7.8`，准备创建并推送 `v1.7.8` tag 触发发布构建。
## 待处理
...
- 推送 `v1.7.8` tag 后构建 zip / MSI 发布产物，并上传 GitHub Release。
...
1. 推送 `main` 和 `v1.7.8` tag，触发 GitHub Actions 构建 zip / MSI 发布产物。
...
**当前任务**：发布 V1.7.8，包含采集与差异模式入口优化、采集批量操作和安装器体验优化。

**已完成**：V1.0.0 全部功能 + V1.1.0 主界面 UI 全面重设计 + V1.1.0 用户测试反馈修复（12 项问题）+ V1.2.0 代码复审修复（5 项问题）+ V1.6.8 语言下拉、托盘和更新提示修复 + V1.6.9 默认基础模式 / 可选差异模式发布 + V1.6.11 外部链接链路复核修复 + 详情面板备注 / 调试端口展示优化 + 差异模式二级管理窗口 + V1.7.3 逐项复核修复 + V1.7.4 全项目复核优化 + V1.7.5 采集模式 + V1.7.6 环境模式窗口布局修复 + V1.7.7 安装器体验优化 + V1.7.8 采集与差异模式入口优化。

**未完成**：需推送 `v1.7.8` tag 触发发布构建；cscout 真实采集需安装 Playwright 后实机验证平台页面。

**下一步建议**：推送 `main` 和 `v1.7.8` tag，等待 GitHub Actions 构建 zip / MSI，创建 GitHub Release；随后用 cscout attach 采集模式端口。
