...
> **最后更新**：2026-05-26
> **最后更新人**：Codex
> **最近开发日志**：`06-dev-log.md` 中的 2026-05-26（V1.7.2 发布触发）
> **当前可信度**：Windows 本地 `dotnet build` 通过，待默认浏览器注册和真实外部链接行为验证

## 当前版本

**V1.7.2 待发布** — 在 V1.7.1 基础上继续完善窗口布局持久化与发布收口，并保持默认基础模式 / 可选差异模式设计。

## 当前阶段

当前重点是将 1.7.2 版本提交、打 tag 并触发 GitHub Actions 构建，然后等待 Windows 设备执行默认浏览器注册和真实外部链接行为验证。
...
**当前任务**：Windows 版 ChromeIsolator 应用开发；当前工作区已完成详情面板与差异模式管理交互优化。
...
**未完成**：默认浏览器注册和真实外部 App 链接尚未完成实机验证。

**下一步建议**：在 Windows 设备上运行应用，验证备注有 / 无内容时的详情布局、基础 / 差异模式调试端口显示、差异模式二级管理窗口搜索与运行中禁用状态，以及默认浏览器注册和真实外部链接行为。
