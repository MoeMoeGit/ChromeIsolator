...
> **最后更新**：2026-07-08
> **最后更新人**：Codex
> **最近开发日志**：`06-dev-log.md` 中的 2026-07-08（安装器体验与发布说明优化）
> **当前可信度**：采集模式、环境模式窗口布局、安装器构建和版本号已完成本机构建验证；真实抖音后台采集需由 cscout + Playwright 实机验证。

## 当前版本

**V1.7.7 待发布** — 在 V1.7.6 基础上优化安装器产品体验：完成页默认启动应用，品牌名统一为“浏览器多开”，替换 WiX 默认图片，强化卸载保留数据和运行中退出提示，并补充 README / Release 安装指引。

## 当前阶段

V1.7.7 安装器体验优化已完成本地构建验证。下一步是推送 `main` 和 `v1.7.7` tag，触发 GitHub Actions 构建；随后由 cscout 连接采集模式环境验证抖音后台真实采集。
...
- 已实现停止操作的异步化与批量停止性能优化，消除线程池阻塞风险。
- 已增加进程关闭操作的安全捕获机制，提升批量环境清理的稳定性。
- 已新增 MSI 官方下载的网络容错重试机制，改善弱网下载可用性。
- 已调整单实例通信的超时与重试策略，适应高负载下的外部链接传递。
- 已新增独立采集模式 `EnableCollectorDebug`：启用后环境启动时开放 `41000 + 环境编号` 的本机 CDP 端口，且显式绑定 `127.0.0.1`。
- 已保持差异模式 `EnableEnvironmentVariation` 独立：不勾选差异模式时不会注入 `navigator.hardwareConcurrency` / `navigator.deviceMemory`。
- 已更新环境模式二级窗口：可分别勾选“采集模式 / 调试端口常开”和“差异模式”，运行中的环境仍需关闭后切换。
- 已优化环境模式二级窗口布局：环境名和运行提示单独一行，模式开关换到下一行并支持换行，避免多语言长文案挤压环境名。
- 已将版本号推进到 `1.7.6`，并已创建 / 推送 `v1.7.6` tag 触发发布构建。
- 已优化 Windows MSI 安装器体验：完成页默认启动应用、继续创建桌面和开始菜单快捷方式、产品名统一为“浏览器多开”、运行中提示明确从托盘右键退出。
- 已替换 WiX 默认安装器图片为 `installer/assets/` 下的项目自有位图，并补充 README / GitHub Release 安装指引。
- 已强化卸载说明：卸载默认只移除程序文件和快捷方式，保留 `%LOCALAPPDATA%\ChromeIsolator` 环境数据。
- 已将版本号推进到 `1.7.7`，准备创建并推送 `v1.7.7` tag 触发发布构建。
## 待处理
...
- 推送 `v1.7.7` tag 后构建 zip / MSI 发布产物，并上传 GitHub Release。
...
| 早期安装包未签名 | 安装时可能出现 SmartScreen / 未知发布者提示 | 已接受 | README 和 GitHub Release 说明已补充处理方式 |

## 下一步

1. 推送 `main` 和 `v1.7.7` tag，触发 GitHub Actions 构建 zip / MSI 发布产物。
...
**当前任务**：发布 V1.7.7，包含采集模式 / 调试端口常开、环境模式窗口布局修复和安装器体验优化。

**已完成**：V1.0.0 全部功能 + V1.1.0 主界面 UI 全面重设计 + V1.1.0 用户测试反馈修复（12 项问题）+ V1.2.0 代码复审修复（5 项问题）+ V1.6.8 语言下拉、托盘和更新提示修复 + V1.6.9 默认基础模式 / 可选差异模式发布 + V1.6.11 外部链接链路复核修复 + 详情面板备注 / 调试端口展示优化 + 差异模式二级管理窗口 + V1.7.3 逐项复核修复 + V1.7.4 全项目复核优化 + V1.7.5 采集模式 + V1.7.6 环境模式窗口布局修复 + V1.7.7 安装器体验优化。

**未完成**：需推送 `v1.7.7` tag 触发发布构建；cscout 真实采集需安装 Playwright 后实机验证平台页面。

**下一步建议**：推送 `main` 和 `v1.7.7` tag，等待 GitHub Actions 构建 zip / MSI，创建 GitHub Release；随后用 cscout attach 采集模式端口。

**风险 / 阻塞**：采集模式只保证本机 CDP 通道可用，不保证平台页面解析成功；差异模式曾导致抖音后台拒绝访问，实采时应只勾采集模式，不勾差异模式。
