...
> **最后更新**：2026-05-25
> **最后更新人**：Codex
> **最近开发日志**：`06-dev-log.md` 中的 2026-05-25（评审复核与持久化修复）
> **当前可信度**：Windows 本地 `dotnet build` 通过，待默认浏览器注册和真实外部链接行为验证

## 当前版本

**V1.6.11 待发布** — 在 V1.6.9 基础上新增环境备注和外部链接接收能力，并继续保留默认基础模式 / 可选差异模式设计。

## 当前阶段

当前重点是将环境备注与外部链接改动推进为新版本，触发 GitHub Actions 构建，并等待 Windows 设备执行默认浏览器注册和真实外部链接行为验证。
...
**当前任务**：Windows 版 ChromeIsolator 应用开发；当前工作区为环境备注与外部链接功能的新版本复核修复。

**已完成**：V1.0.0 全部功能 + V1.1.0 主界面 UI 全面重设计 + V1.1.0 用户测试反馈修复（12 项问题）+ V1.2.0 代码复审修复（5 项问题）+ V1.6.8 语言下拉、托盘和更新提示修复 + V1.6.9 默认基础模式 / 可选差异模式发布 + V1.6.11 外部链接链路复核修复。

**未完成**：默认浏览器注册和真实外部 App 链接尚未完成实机验证。

**下一步建议**：在 Windows 设备上编译运行，验证备注编辑、设置页外部链接下拉、设为默认浏览器请求、冷启动外部链接、运行中环境追加 URL 和浏览器未就绪提示。
