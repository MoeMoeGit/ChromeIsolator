...
> **最后更新**：2026-05-25
> **最后更新人**：Codex
> **最近开发日志**：`06-dev-log.md` 中的 2026-05-25（环境备注与外部链接）
> **当前可信度**：代码已静态自查，待 Windows 实机编译验证

## 当前版本

**V1.6.10 待发布** — 在 V1.6.9 基础上新增环境备注和外部链接接收能力，并继续保留默认基础模式 / 可选差异模式设计。

## 当前阶段

当前重点是将环境备注与外部链接改动推进为新版本，触发 GitHub Actions 构建，并等待 Windows 设备执行 WPF 编译和实机行为验证。
...
**当前任务**：Windows 版 ChromeIsolator 应用开发；当前工作区为环境备注与外部链接功能的新版本触发改动。

**已完成**：V1.0.0 全部功能 + V1.1.0 主界面 UI 全面重设计 + V1.1.0 用户测试反馈修复（12 项问题）+ V1.2.0 代码复审修复（5 项问题）+ V1.6.8 语言下拉、托盘和更新提示修复 + V1.6.9 默认基础模式 / 可选差异模式发布。

**未完成**：环境备注与外部链接尚未在 Windows 设备上执行 `dotnet build`、默认浏览器注册和真实外部 App 链接验证。
