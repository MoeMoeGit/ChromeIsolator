...
- **基础模式**：关闭采集和差异后，不启用调试端口、不注入页面脚本，降低特定网站兼容问题；可在设置中为已关闭的指定环境单独启用“差异模式”
- **采集模式 / 调试端口常开**：新建环境（含首次生成）默认开启，可在设置中对已关闭的环境单独关闭或启用本机 CDP 调试端口，端口为 `41000 + 环境编号`，便于 cscout 连接已登录环境；cscout 会自行创建后台标签页并导航，不要求用户提前打开具体页面。该模式不会自动启用差异注入
...
- **设置面板**：查看浏览器引擎状态和版本、打开数据目录、复制路径、浏览器引擎设置、采集与差异模式管理、外部链接默认目标、设为默认浏览器、高级详情显示、语言切换，以及帮助与更新信息
- **多语言支持**：内置中文、English、日本語 三种语言，自动检测系统语言，可在设置中切换
...
核心功能已完成，包括环境管理、环境备注、外部链接接收、浏览器引擎设置、可选采集模式、可选差异模式、多语言、设置面板、系统托盘和 MSI 安装包构建。当前版本为 V1.7.12，项目继续验证采集模式、默认浏览器注册、真实外部链接行为和本轮稳定性优化。
...
artifacts\publish\ChromeIsolator-win-x64-v1.7.12.zip
```
...
```text
artifacts\installer\ChromeIsolator-Setup-x64-v1.7.12.msi
```

## 版本管理

项目版本统一写在根目录 `Directory.Build.props`。发布新版本时只修改其中的 `Version`、`AssemblyVersion`、`FileVersion` 和 `InformationalVersion`，然后创建同版本 tag，例如 `v1.7.12`。
