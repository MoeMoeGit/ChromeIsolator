...
- **快速启动和关闭**：在主窗口或系统托盘启动、关闭单个环境，也可以一次关闭全部环境；支持"全部关闭并退出"，退出前自动等待 Chrome 进程结束
- **双击启动**：双击环境列表项直接启动或关闭环境
- **右键菜单**：右键环境列表可快速重命名或删除环境
- **智能排序**：运行中的环境自动置顶，最近使用的排在前面；显示启动中/关闭中状态
- **专业管理界面**：左侧环境列表用于快速扫描，右侧详情栏展示路径、环境差异模式调试端口、磁盘占用、错误和高级信息；支持打开环境目录和复制路径
- **自定义命名**：新增环境后自动弹出重命名对话框，方便立即命名
- **安全删除**：删除环境需要输入 `delete` 确认，对话框显示数据大小，数据会先移到 Windows 回收站
- **错误恢复**：环境启动失败时可重试或清除错误状态
- **兼容优先**：默认不启用调试端口、不注入页面脚本，降低特定网站兼容问题；可在设置中为已关闭的指定环境单独启用“环境差异模式”
- **轻量环境差异**：环境差异模式会为指定环境注入稳定的 `navigator.hardwareConcurrency` 和 `navigator.deviceMemory` 值，自动处理新打开的标签页，断线后自动重连（最多 5 次指数退避）
...
核心功能已完成，包括环境管理、浏览器引擎准备、可选环境差异模式、多语言、设置面板、系统托盘和 MSI 安装包构建。项目正在验证 MSI 安装体验和完善细节。
